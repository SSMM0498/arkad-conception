...
Concerning the CSS Style i's a priority to studies the case of `:hover` selector. In fact,  we can't trigger the hover state via JavaScript and in addition a mock mouse cursor is use during the replay. So when playing back we need to simulate the hover state to make the style display correctly.
